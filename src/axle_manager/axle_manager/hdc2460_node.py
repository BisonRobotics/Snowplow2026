...
class Hdc2460Node(Node):
    def __init__(self):
        ...
        # Start subscriptions
        self.speedSubscription = self.create_subscription(
            Twist, "/cmd_vel", self.speed, 10
        )
        self.speedSubscription  # prevent unused variable warning
        self.pivotSubscription = self.create_subscription(
            Int8, "/vehicle/pivot", self.pivot, 10
        )
        self.pivotSubscription  # prevent unused variable warning
        self.plowSubscription = self.create_subscription(
            Twist, "/vehicle/plow", self.plow, 10
        )
    ...
    def speed(self, msg: Twist):
        ...
        self.get_logger().debug(
            "Roboteq: Left={0:.2f} Right={0:.2f}".format(left_speed, right_speed)
        )
    ...
    def plow(self, msg: Twist):
        ...
        self.get_logger().debug(
            "Roboteq: Val={0:.4f} Ch1={0} Ch2={0}".format(cmd.x, chR, chL)
        )
        self.get_logger().debug(
            "Roboteq: Val={0:.4f} Ch1={0} Ch2={0}".format(cmd.y, chU, chD)
        )
    ...
    def publish_sensor_data(self):
        ...
        if self.pivotDevice.casefold() == "FAC".casefold():
            ...
            self.get_logger().warning("No pivot device configured.")
        self.get_logger().debug(
            "Roboteq: AI={0:.2f} Ch={1}".format(pivot.data, self.pivotSensor)
        )
