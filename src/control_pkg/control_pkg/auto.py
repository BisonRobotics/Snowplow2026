...
class Auto(Node):
    def __init__(self):
        ...
        self.create_subscription(
            Float32, "/sensor/pivot", self.update_pivot_position, 10
        )
    ...
    def get_auto_command(self) -> Command:
        ...
        turn_to_degrees = lambda degrees: TurnToDegreesCommand(
            degrees, self.get_pivot_position, self.drive_pivot
        )
        drive_to_waypoint = lambda waypoint: DriveToWaypointCommand(
            waypoint,
            self.get_position,
        ...
        drive_distance = lambda speed, distance: DriveDistanceCommand(
            speed, distance, self.drive
        )
        wait_until = lambda condition: WaitUntilCommand(condition)

        # Creating and returning command
        return (
            wait_until(lambda: self.position_updated and self.pivot_position_updated)
