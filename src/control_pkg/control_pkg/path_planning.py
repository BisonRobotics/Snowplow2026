...
def direction(start_point, end_point):
    ...
    return (
        math.pi + math.atan(change[1] / change[0])
        if change[0] < 0
        else math.atan(change[1] / change[0])
    )
...
def pivot_tuple(point_a, point_b, direction):
    ...
    tuple_a = (
        point_b[0] + turn_radius * math.cos(rad_a),
        point_b[1] + turn_radius * math.sin(rad_a),
    )

    # find second tuple
    rad_b = math.radians(direction - 90)
    tuple_b = (
        point_b[0] + turn_radius * math.cos(rad_b),
        point_b[1] + turn_radius * math.sin(rad_b),
    )
    ...
    if math.dist(point_a, tuple_a) <= math.dist(point_a, tuple_b):
        return (
            tuple_a,
            -1,
        )  # turn left one of these could possibly be inverted need to test
    else:
...
def final_direction(tuple, point, direction):
    ...
    tuple_a = (
        tuple[0] + turn_radius * math.cos(rad),
        tuple[1] + turn_radius * math.sin(rad),
    )
    tuple_b = (
        tuple[0] - turn_radius * math.cos(rad),
        tuple[1] - turn_radius * math.sin(rad),
...
def turn_path(start_point, start_direction, end_point, end_direction):
    ...
    end_tuplea = (
        math.cos(math.radians(end_direction + 90)),
        math.sin(math.radians(end_direction + 90)),
    )
    end_tuplea = (
        end_point[0] + turn_radius * end_tuplea[0],
        end_point[1] + turn_radius * end_tuplea[1],
    )
    ...
    end_tupleb = (
        end_point[0] + turn_radius * end_tupleb[0],
        end_point[1] + turn_radius * end_tupleb[1],
    )
    ...
    if math.dist(start_point, end_tuplea) <= math.dist(start_point, end_tupleb):
        ...
        end_turn = (
            -1
        )  # turn left one of these could possibly be inverted need to test (flipped >)
    else:
    ...
    start_tuplea = (
        math.cos(math.radians(start_direction + 90)),
        math.sin(math.radians(start_direction + 90)),
    )
    start_tuplea = (
        start_point[0] + turn_radius * start_tuplea[0],
        start_point[1] + turn_radius * start_tuplea[1],
    )
    ...
    start_tupleb = (
        start_point[0] + turn_radius * start_tupleb[0],
        start_point[1] + turn_radius * start_tupleb[1],
    )
    ...
    if start_turn == end_turn:
        ...
        tan_point_starta = (
            start_tuple[0]
            + turn_radius * (math.cos(direction(start_tuple, end_tuple) - pi / 2)),
            start_tuple[1]
            + turn_radius * (math.sin(direction(start_tuple, end_tuple) - pi / 2)),
        )  # counterclockwise
        ...
        tan_point_endb = (
            end_tuple[0]
            + turn_radius * (math.cos(direction(end_tuple, start_tuple) - pi / 2)),
            end_tuple[1]
            + turn_radius * (math.sin(direction(end_tuple, start_tuple) - pi / 2)),
        )  # counterclockwise
        ...
        if (
            abs(
                direction(start_tuple, tan_point_starta)
                - direction(start_tuple, start_point)
                + math.radians(start_direction)
                - direction(tan_point_starta, tan_point_enda)
            )
            % (2 * math.pi)
            <= 0.0001
        ):
        ...
        short = 2 * turn_radius
        theta = (
            math.atan2(end_tuple[1] - start_tuple[1], end_tuple[0] - start_tuple[0])
            + math.asin(short / hypotenuse)
            - pi / 2
        )
        tan_point_starta = (
            start_tuple[0] + turn_radius * math.cos(theta),
            start_tuple[1] + turn_radius * math.sin(theta),
        )
        tan_point_enda = (
            end_tuple[0] + turn_radius * math.cos(theta + pi),
            end_tuple[1] + turn_radius * math.sin(theta + pi),
        )
        if (
            abs(
                (
            ...
            if end_tuple[0] - start_tuple[0] == 0:
                ...
                slope = (end_tuple[1] - start_tuple[1]) / (
                    end_tuple[0] - start_tuple[0]
                )
                ...
                if slope != 0:
                    ...
                    intersectionstart = (
                        intersectionstart[0],
                        intersectionstart[0] * slope + b1,
                    )
                    intersectionend = (
                        intersectionend[0],
                        intersectionend[0] * slope + b1,
                    )
                    tan_point_startb = (
                        2 * intersectionstart[0] - tan_point_starta[0],
                        2 * intersectionstart[1] - tan_point_starta[1],
                    )
                    tan_point_endb = (
                        2 * intersectionend[0] - tan_point_enda[0],
                        2 * intersectionend[1] - tan_point_enda[1],
                    )
                else:
                    tan_point_startb = (
                        tan_point_starta[0],
                        start_tuple[1] * 2 - tan_point_starta[1],
                    )
                    tan_point_endb = (
                        tan_point_enda[0],
                        end_tuple[1] * 2 - tan_point_enda[1],
                    )
    ...
    placeholder_tuple1 = (
        start_tuple[0] + turn_radius * math.cos(math.radians(start_direction)),
        start_tuple[1] + turn_radius * math.sin(math.radians(start_direction)),
    )
    placeholder_tuple2 = (
        start_tuple[0] - turn_radius * math.cos(math.radians(start_direction)),
        start_tuple[1] - turn_radius * math.sin(math.radians(start_direction)),
    ...
    if math.dist(tan_point_start, placeholder_tuple1) <= math.dist(
        tan_point_start, placeholder_tuple2
    ):
    ...
    if math.dist(
        tan_point_end,
        (
            end_tuple[0] - turn_radius * math.cos(math.radians(end_direction)),
            end_tuple[1] - turn_radius * math.sin(math.radians(end_direction)),
        ),
    ) <= math.dist(
        tan_point_end,
        (
    ...
    if start_turn == -1:
        distance1 = round(
            ...
            * (
                round(
                    (
                        direction(start_tuple, tan_point_start)
                        - direction(start_tuple, start_point)
                    ),
                    3,
                )
    ...
    if end_turn == -1:
        distance3 = round(
            ...
            * (
                round(
                    (
                        direction(end_tuple, end_point)
                        - direction(end_tuple, tan_point_end)
                    ),
                    3,
                )
                % (2 * pi)
    ...
    return (
        start_turn,
        direction_movement_start,
        distance1,
        0,
        1,
        distance2,
        end_turn,
        direction_movement_end,
        distance3,
    )
...
if __name__ == "__main__":
    ...
    (
        ...
        distance1,
        mid_turn,
        direction_movement_mid,
        distance2,
        end_turn,
        direction_movement_end,
        distance3,
    ) = turn_path((0.14, -2.5), 90, (-2.5, 2), 180)
