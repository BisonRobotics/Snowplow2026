...
class DriveDistanceCommand(DriveTimeCommand):
    ...
    def __init__(self, speed: float, distance: float, drive: Callable[[float], None]):
        super().__init__(
            speed=speed, drive_time=self.calculate_time(speed, distance), drive=drive
        )
    ...
    def calculate_time(self, speed, distance) -> float:
        ...
        if distance < real_speed * (acceleration_time + deceleration_time) / 2:
            return (
                sqrt(
                    (2 * distance * acceleration * deceleration)
        ...
        hold_time = (
            (2 * distance / real_speed) - acceleration_time - deceleration_time
        ) / 2
...
class DriveToWaypointCommand(SequentialCommandGroup):
    ...
    def __init__(
        self,
        waypoint: Twist,
        get_position: Callable[[], Twist],
        get_pivot_position: Callable[[], float],
        drive_pivot: Callable[[int], None],
    ...
    def initialize(self):
        ...
        path = turn_path(
            start_point=(my_position.linear.x, my_position.linear.z),
            start_direction=my_position.angular.y,
            end_point=(self.waypoint.linear.x, self.waypoint.linear.z),
            end_direction=self.waypoint.angular.y,
        )

        path_segment_1 = SequentialCommandGroup()
        path_segment_1.add_commands(
            TurnToDegreesCommand(
                path[0] * 18.624, self.get_pivot_position, self.drive_pivot
            ),
            WaitCommand(2),
        ...
        path_segment_2.add_commands(
            TurnToDegreesCommand(
                path[3] * 18.624, self.get_pivot_position, self.drive_pivot
            ),
            WaitCommand(2),
        ...
        path_segment_3.add_commands(
            TurnToDegreesCommand(
                path[6] * 18.624, self.get_pivot_position, self.drive_pivot
            ),
            WaitCommand(2),
