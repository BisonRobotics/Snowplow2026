...
def generate_launch_description():
    ...
    launchfile = os.path.basename(__file__)[
        :-3
    ]  # convert "<lidar_name>.launch.py" to "<lidar_name>.launch"
    launch_file_path = os.path.join(
        sick_scan_pkg_prefix, "launch/" + launchfile
    ...
    if (
        ROS_DISTRO[0] <= "e"
    ):  # ROS versions eloquent and earlier require "node_executable", ROS foxy and later use "executable"
        node = Node(
            package="sick_scan",
