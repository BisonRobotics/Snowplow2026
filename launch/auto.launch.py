...
def generate_launch_description():
    # Auto control node
    start_auto_node = Node(package="control_pkg", executable="auto", name="auto")
    ...
    start_axle_manager_node = Node(
        ...
        arguments=[
            "--params-file "
            + os.path.join(
                get_package_share_directory("axle_manager"), "config", "params.yaml"
            )
        ],
    )

    # VN100 Sensor IMU Node
    start_imu_node = Node(package="sensors_pkg", executable="imu", name="imu")
